...
themeSwitch.addEventListener('change', e => {
  if (e.target.matches('#dark-mode')) {
      ...
      for (let i = 0; i < whiteTxt.length; i++) whiteTxt[i].classList.toggle('white')
      for (let i = 0; i < blueTxt.length; i++) blueTxt[i].classList.toggle('desat-blue')
      for (let i = 0; i < gridItems.length; i++) gridItems[i].classList.toggle('grid-item-dark')
